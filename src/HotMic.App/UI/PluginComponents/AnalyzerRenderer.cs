...
public sealed class AnalyzerRenderer : IDisposable
{
    ...
    public SpectroHitTest HitTest(float x, float y)
    {
        ...
        if (_formantToggleRect.Contains(x, y)) return new SpectroHitTest(SpectroHitArea.FormantToggle, -1);
        ...
        if (_formantProfileRect.Contains(x, y)) return new SpectroHitTest(SpectroHitArea.FormantProfileButton, -1);
        ...
        if (_vowelToggleRect.Contains(x, y)) return new SpectroHitTest(SpectroHitArea.VowelToggle, -1);
    ...
    private void DrawVowelSpace(SKCanvas canvas, SKRect rect, SpectroState state, int frameIndex)
    {
        canvas.DrawRoundRect(new SKRoundRect(rect, 6f), _panelPaint);
